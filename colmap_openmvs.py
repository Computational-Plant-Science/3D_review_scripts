...
time python3 colmap_openmvs.py -p ~/example/B101/
...
def colmap_vsfm_pipeline(file_path):
    ...
    feature_extract = "colmap feature_extractor --image_path " + file_path + " --database_path " + file_path + "/database.db " + "--SiftExtraction.use_gpu=True"
    execute_script(feature_extract)
    
    feature_matching = "colmap exhaustive_matcher --database_path " + file_path + "/database.db" + " --SiftMatching.use_gpu=True"
    execute_script(feature_matching)
    
    create_folder_sparse = "mkdir " + file_path + "/sparse" 
    execute_script(create_folder_sparse)
    
    #create_folder_sparse = "mkdir " + file_path + "/sparse/0" 
    #execute_script(create_folder_sparse)
    ...
    dense_model = "~/vsfm/bin/VisualSFM sfm+loadnvm+pmvs " + file_path + "/model.nvm " + file_path + "/dense.nvm "
    execute_script(dense_model)
    
    #permission_change = "chmod 777 -R /srv/images/"
    
    permission_change = "chmod 777 -R " + file_path
